...
io.write("Enter the number of hidden nodes for BinaryNet Model:\n")
local numHid = io.read("*n")
io.write("Enter the number of features for BinaryNet Model:\n")
local num_variables = io.read("*n")

-- Convolution Layers
model:add(nn.View(-1,num_variables))

model:add(BinaryLinear(num_variables,numHid))
...
--model:add(nn.HardTanh())
--model:add(BinarizedNeurons(opt.stcNeurons))
model:add(BinaryLinear(numHid,2,opt.stcWeights))
--model:add(nn.BatchNormalization(2))
