...
elseif Dataset == 'MNIST8x8' then
  io.write("Enter the number of features for Data.lua:\n")  
  num_features = io.read("*n")
  local file_valid = paths.concat(PreProcDir, 'mnist_8x8_' ..num_features .. '_valid.t7')
  local file_train = paths.concat(PreProcDir, 'mnist_8x8_' .. num_features .. '_train.t7')
  local file_test = paths.concat(PreProcDir, 'mnist_8x8_' .. num_features .. '_test.t7')
  ValidData=torch.load(file_valid)
  TrainData=torch.load(file_train)
  TestData=torch.load(file_test)
  Classes = {1,2}--,3,4,5,6,7,8,9,0}
